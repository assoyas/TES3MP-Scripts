...
Methods.LoadVampirism = function(pid)
    ...
    for index, item in pairs(spellbook) do
        if spellbook[index].spellId == "vampire berne specials" then
            vampireClan = "berne"
            break
        elseif spellbook[index].spellId == "vampire aundae specials" then
            vampireClan = "aundae"
            break
        elseif spellbook[index].spellId == "vampire quarra specials" then
            vampireClan = "quarra"
            break
        else
            vampireClan = "none"
    ...
    if vampireClan ~= "none" then
        myMod.RunConsoleCommandOnPlayer(pid, "player->addspell \"vampire blood " .. vampireClan .. "\"")
        myMod.RunConsoleCommandOnPlayer(pid, "startscript, \"vampire_" .. vampireClan .. "_PC\"")
    end
end
...
--[[If server uses synced journal, we want to run the Startup script once when the server is first "intialized"
Therefore, we check if none of the startup cells have json files yet; in that case, startup script is executed via console
which creates the files and prevents the script from ever being run again
The objects will then be re-enabled in a normal fashion as players progress through quests]]
