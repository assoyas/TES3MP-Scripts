...
Methods.ShowPlayTime = function(pid)
    local timeString = Methods.GetTimeString(pid, false)
    local message = "You have been playing for " .. timeString
    tes3mp.MessageBox(pid, -1, message)
end

Methods.GetConnectedPlayersPlayTime = function()
    ...
    for i = 0, lastPid do
        ...
        if Players[i] ~= nil and Players[i]:IsLoggedIn() then
            local timeString = Methods.GetTimeString(i, true)
...
Methods.ShowPlayTimeAllConnected = function(pid)
    local label = "Connected players' play time"
    tes3mp.ListBox(pid, playTimeGuiID, label, Methods.GetConnectedPlayersPlayTime())
end

Methods.GetTimeString = function(pid, returnList)
    local playTime = Players[pid].data.customVariables.playTime
    local timeString = ""
    local mod
